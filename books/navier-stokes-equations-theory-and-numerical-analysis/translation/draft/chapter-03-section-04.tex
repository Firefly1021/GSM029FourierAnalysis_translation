...
\hypertarget{chapter-03-page-239}{}
对每个固定的 $t$, $u_{k'}(t)$ 在 $H$ 中关于 $k'$ 和 $t$ 一致有界; 当 $k'\to0$ 时, $u_{k'}(t)$ 在 $V_s'$ 中弱收敛到 $u(t)$. 因此\footnote{用反证法证明.}
\begin{equation}
u_{k'}(t)\rightharpoonup u(t)
\quad\text{在 }H\text{ 中},
\qquad k'\to0,\quad\forall t\in[0,T].
\label{eq:ch3-semidiscrete-pointwise-weak-h}
\end{equation}
随后在 \eqref{eq:ch3-semidiscrete-functional-energy-remark} 中取下极限($t$ 固定, $k'\to0$), 并使用 \eqref{eq:ch3-semidiscrete-uk-weak-limits} 和 \eqref{eq:ch3-semidiscrete-pointwise-weak-h}, 得到能量不等式
\begin{equation}
|u(t)|^2+2\nu\int_0^t\lVert u(s)\rVert^2\,ds
\leq|u_0|^2+2\int_0^t\langle f(s),u(s)\rangle\,ds,
\qquad\forall t\in[0,T].
\label{eq:ch3-semidiscrete-limit-energy-inequality}
\end{equation}
若 $n=2$, 利用 \eqref{eq:ch3-uniqueness-2d-energy-derivative}, 容易直接证明能量等式
\begin{equation}
|u(t)|^2+2\nu\int_0^t\lVert u(s)\rVert^2\,ds
=|u_0|^2+2\int_0^t\langle f(s),u(s)\rangle\,ds,
\qquad\forall t\in[0,T].
\label{eq:ch3-semidiscrete-limit-energy-equality-2d}
\end{equation}
