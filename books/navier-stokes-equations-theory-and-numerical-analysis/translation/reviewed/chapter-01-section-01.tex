...
在第 \ref{sec:ch1-function-spaces} 节中, 我们考虑若干函数空间, 即分量属于 $L^2$ 的无散向量函数空间. 第 \MBUnresolvedReference{section}{2} 节给出 Stokes 方程的变分表述, 并利用投影定理证明解的存在性与唯一性. 在第 \MBUnresolvedReference{section}{3} 节和第 \MBUnresolvedReference{section}{4} 节中, 我们回顾赋范空间和变分线性方程逼近的若干定义与结果. 随后提出无散向量函数的一个基本空间 $V$ 的几类逼近, 其中包括有限差分逼近以及协调和非协调有限元逼近. 第 \MBUnresolvedReference{section}{5} 节讨论 Stokes 方程的若干逼近算法及相应的离散方程. 这些算法旨在克服条件 $\operatorname{div}u=0$ 所造成的困难. 正如后文将表明的, 有时这一困难并不能仅靠离散化解决.

最后, 第 \MBUnresolvedReference{section}{6} 节研究微可压缩流体的线性化方程, 以及它们向不可压缩流体线性方程(即 Stokes 方程)的渐近收敛.
...
\]
另见 \MBUnresolvedReference{equation}{(6.12)} 中 \eqref{eq:ch1-gradient-hminus1-bound} 的另一版本.
\end{Remark}
