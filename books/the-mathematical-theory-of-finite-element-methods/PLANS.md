...
5. Current phase: full-book translation. `chapter-00` through `chapter-14` are complete and passed; the next natural unit is `frontmatter`.
...
8. The user resumed the task after the Chapter 5 checkpoint; all numbered chapters are now passed, so continue with frontmatter, references and index without revisiting passed units.
