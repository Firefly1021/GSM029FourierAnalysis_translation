...
\par\noindent\textbf{Remark:}\quad\label{rem:chap2-small-space-weak-star}
上述结果容易推广到弱-* 收敛情形.
...
\par\noindent\textbf{Remark:}\quad\label{rem:chap2-weak-products}
...
\par\noindent\textbf{Remark:}\quad\label{rem:chap2-minkowski-norm}
...
\begin{Proof}
令 $p\geq2$ 时 $\alpha=1$, $p<2$ 时 $\alpha=1/(p-1)$. 将 Lemma~\ref{lem:chap2-clarkson-inequalities} 应用于 $u_n$ 和 $u$, 得
\begin{equation}
 \limsup_{n\to\infty}a_n\leq\|u\|_{L^p}^{\alpha p},
...
$L^p_{\mathrm{loc}}(\Omega)$ 表示绝对值的 $p$ 次幂在 $\Omega$ 的每个紧子集上可积的可测函数空间. $L^\infty_{\mathrm{loc}}(\Omega)$ 表示在每个这样的紧集上本质有界的可测函数空间.
\end{Definition}
...
\begin{Proof}
由 Propositions~\ref{prop:chap2-bounded-lp-weak-subsequence} 和 \ref{prop:chap2-bounded-linfty-weak-star-subsequence}, 可抽取子序列在 $L^q(\Omega)$ 中弱收敛 (若 $q=+\infty$, 则弱-$*$ 收敛) 到 $v\in L^q(\Omega)$. 局部强收敛说明 $u=v$. 令 $\omega_k=\{x\in\Omega:d(x,\partial\Omega)>1/k\}$. 则 $\|u_n-u\|_{L^p(\omega_k)}\to0$, 且
\[
 \|u_n-u\|_{L^p(\Omega\setminus\omega_k)}
 \leq2C|\Omega\setminus\omega_k|^{(q-p)/(pq)}.
\]
先令 $k\to\infty$, 再令 $n\to\infty$.
\end{Proof}
...
\begin{Proof}
令 $\delta=d(\overline\omega,\mathbb{R}^d\setminus\Omega)>0$ 及 $U=\{x\in\Omega:d(x,\omega)<\delta/2\}$. 则 $\mathbf{1}_U*\eta_{\delta/4}$ 满足结论.
\end{Proof}
...
\begin{Proof}
取可数球族 $(B_n)_n$ 覆盖 $A$, 每个球都包含于某个 $\omega_i$ 中; 令 $V_n$ 为同心半径减半的球. 由 Lemma~\ref{lem:chap2-cutoff-function}, 取 $\varphi_n\in C_c^\infty(B_n)$, $0\leq\varphi_n\leq1$, 且 $\varphi_n=1$ 于 $V_n$. 定义
\[
 \alpha_0=\varphi_0,\qquad
 \alpha_n=(1-\varphi_0)\cdots(1-\varphi_{n-1})\varphi_n.
\]
则
\begin{equation}
 \sum_{n\in\mathbb{N}}\alpha_n(x)=1,\qquad\forall x\in A,
 \label{eq:chap2-alpha-partition}
\end{equation}
且该和局部有限. 把支集落在同一 $\omega_i$ 中的 $\alpha_n$ 分组相加, 即得 $(\psi_i)_{i\in I}$.
\end{Proof}
...
连续线性映射 $T:\mathcal{D}(\Omega)\to\mathbb{R}$ 称为分布. 分布空间记为 $\mathcal{D}'(\Omega)$, 并记 $\langle T,\varphi\rangle_{\mathcal{D}',\mathcal{D}}=T(\varphi)$.
\end{Definition}
...
\begin{Proof}
若 $T_f=T_g$, 对 $\overline\omega\subset\Omega$ 令 $h=\operatorname{sgn}(f-g)$. 由 Theorem~\ref{thm:chap2-test-functions-weak-star-dense-linfty}, 取 $\varphi_n\in\mathcal{D}(\omega)$ 弱-$*$ 收敛到 $h$. 于是 $\int_\omega(f-g)\varphi_n=0$, 令 $n\to\infty$ 得 $\int_\omega|f-g|=0$. 因 $\omega$ 任意, $f=g$ 几乎处处. 若 $f_n\to f$ 于 $L^1_{\mathrm{loc}}$, 则对每个测试函数 $\varphi$, $\varphi f_n\to\varphi f$ 于 $L^1$, 故 $T_{f_n}\to T_f$.
\end{Proof}

由此把 $L^1_{\mathrm{loc}}(\Omega)$ 视为 $\mathcal{D}'(\Omega)$ 的子空间. 对充分光滑的 $f$, 分布导数与经典导数一致.
...
\begin{Proof}
先设 $\Omega=(0,1)^d$. 固定非负 $\theta\in\mathcal{D}((0,1))$ 且 $\int_0^1\theta=1$. 对 $\varphi\in\mathcal{D}(\Omega)$ 逐坐标积分, 并依次使用 $\partial T/\partial x_i=0$, 得
\[
 \langle T,\varphi\rangle
 =\langle T,\theta(x_1)\cdots\theta(x_d)\rangle\int_\Omega\varphi.
\]
平移和位似给出任意立方体情形. 对一般连通 $\Omega$, 取局部有限的立方体覆盖及由 Lemma~\ref{lem:chap2-partition-of-unity} 给出的单位分解. $T$ 在每个立方体上为常数 $\alpha_i$, 因而与光滑函数 $\sum_i\alpha_i\psi_i$ 相同. 该函数梯度为零, 由连通性知其为常数.
\end{Proof}
...
\begin{Proof}
令 $L=\operatorname{Lip}(f)$, 则 $F(x)=\inf_{y\in A}(f(y)+L|x-y|)$ 满足要求.
\end{Proof}
...
设 $f$ 在 $\mathbb{R}^d$ 上 Lipschitz 连续. 则 $\operatorname{Lip}(f*\eta_\varepsilon)\leq\operatorname{Lip}(f)$; 当 $\varepsilon\to0$ 时, $f*\eta_\varepsilon$ 一致收敛到 $f$; 且在 $f$ 的每个可微点 $x$,
\[
...
前两项由
\[
 |f*\eta_\varepsilon(x)-f*\eta_\varepsilon(y)|\leq\operatorname{Lip}(f)|x-y|,
\]
以及
\[
...
得到. 若 $f$ 在 $x$ 可微, 写成
\[
 |f(x+h)-f(x)-\nabla f(x)\cdot h|\leq|h|\tau(h),\qquad\tau(h)\to0.
\]
由式\eqref{eq:chap2-mollified-convolution} 以及
\[
 \int_B\partial_{z_i}\eta=0,\qquad \int_Bz\partial_{z_i}\eta=-e_i,
\]
得到
\[
 |\partial_{x_i}(f*\eta_\varepsilon)(x)-\partial_{x_i}f(x)|
 \leq\int_B|z|\,|\tau(\varepsilon z)|\,|\partial_{z_i}\eta(z)|\,\mathrm{d}z\longrightarrow0.
\]
\end{Proof}
