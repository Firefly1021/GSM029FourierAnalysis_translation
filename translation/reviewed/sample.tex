...
% Formula transcriptions remain blocking-needs-review against registered crops.
...
上一章的可和方法可以看作恒等逼近.对 Cesàro 可和法,$\varphi=F_1$ 且 $F_R=\varphi_{1/R}$(见 \MBUnresolvedReference{equation}{(1.24)});对 Abel--Poisson 可和法,$\varphi=P_1$(见 \MBUnresolvedReference{equation}{(1.30)});对 Gauss--Weierstrass 可和法,$\varphi=W_1$(见 \MBUnresolvedReference{equation}{(1.31)}).由下述结果可见,这些可和方法在 $L^p$ 范数下收敛.
...
\]
\end{Corollary}
特别地,\MBUnresolvedReference{section}{第 1 章第 9 节} 讨论的 Cesàro、Abel--Poisson 与 Gauss--Weierstrass 可和法,对上述任一空间中的 $f$ 都几乎处处收敛到 $f(x)$.

\begin{Proof}
因为对 $f\in\mathcal S$ 已有收敛,由定理~\ref{thm:ch2-closed-ae-convergence-set},对 $f\in L^p$(若 $p=\infty$,则对 $f\in C_0$)也有收敛.Poisson 核 \MBUnresolvedReference{equation}{(1.30)} 与 Gauss--Weierstrass 核 \MBUnresolvedReference{equation}{(1.31)} 递减;Fejér 核 \MBUnresolvedReference{equation}{(1.24)} 不递减,但 $F_1(x)\le\min(1,(\pi x)^{-2})$.
\end{Proof}
